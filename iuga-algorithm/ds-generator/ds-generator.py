...
with open('arquivo.csv') as f1:
    for row in csv.reader(iter(f1.readline, '')):
        if(firstline == False): # Ignore header line
            ...
            #latpickparts = row[latpick].split(".")
            #longpickparts = row[longpick].split(".")
            #latdropparts = row[latdrop].split(".")
            #longdropparts = row[longdrop].split(".")
            #pickuppoint = str(int(latpickparts[0])+180) + str(int(longpickparts[0]) + 180)
            #dropoffpoint = str(int(latdropparts[0]) +180) + str(int(longdropparts[0]) + 180)
            #pickuppoint = int(pickuppoint)
            #dropoffpoint = int(dropoffpoint)
